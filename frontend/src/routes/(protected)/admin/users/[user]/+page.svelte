<script lang="ts">
    ...
    let user = $state<FullUser|null>(null);
    let projects = $state<FullProject[]>([]);

    let loading = $state(true);

    onMount(async () => {
        const res = await apiCall("/api/v1/internal/admin/users/" + userId);

        if(!res.ok) {
            console.error("Failed to fetch user data:", res.statusText);
            console.error("Response:", await res.text());
            toastStore.add("Failed to fetch user data", { type: "error" });
            return;
        }

        const json = await res.json();

        user = json.user as FullUser;
        projects = json.projects as FullProject[];

        loading = user === null || projects === null;
    ...
    <div class="max-w-6xl mx-auto space-y-6">
        ...
        <header class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <nav class="flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400 mb-2">
                    ...
                    <span>/</span>
                    <span class="text-zinc-900 dark:text-white font-medium">{user?.name ?? 'Unknown User'}</span>
                </nav>
        ...
        {#if loading}
            <div class="flex items-center justify-center h-64">
                ...
                <div class="w-16 h-16 border-4 border-t-4 border-red-500 border-t-transparent rounded-full animate-spin"></div>
                <div class="ml-4 text-lg font-semibold text-red-500">Loading User Data...</div>
                ...
                <div class="lg:col-span-1 space-y-6">
                    <Card>
                        <div class="flex flex-col items-center text-center">
                            <UserAvatar user={{name: user?.name ?? "Unknown", avatarId: user?.avatarId}} class="rounded-2xl w-24 h-24 mb-4" />
                                ...
                                {#each projects as project}
                                    <div class="py-3 flex items-center justify-between group">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p class="text-zinc-900 dark:text-white font-medium">{project.name}</p>
                                                <p class="text-xs text-zinc-500 dark:text-zinc-400">Owner</p>
                                            </div>
                                        </div>
                                        <a href={"/admin/projects/" + project.id} class="p-2 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors">
